 ...
 */
public class MaximumFontToFitASentenceInAScreen1618 {
    ...
    public int maxFont(String text, int w, int h, int[] fonts, FontInfo fontInfo) {
        ...
        while(low <= high){
            ...
            if(canFit(text, text.length(), w, h, fonts[mid], fontInfo)){
                low = mid + 1;
            }else{
                high = mid - 1;
            }
    ...
    private boolean canFit(String text, int length, int w, int h, int fontSize, FontInfo fontInfo){
        int totalWidth = 0;
        if(fontInfo.getHeight(fontSize) > h){   // important optimization
            return false;
        }

        for(int i = 0; i < length; i++){
            totalWidth += fontInfo.getWidth(fontSize, text.charAt(i));
